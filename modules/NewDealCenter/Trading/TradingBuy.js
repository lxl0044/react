...
import {createTradingOrderLimit, getDealWaitingPrice, getDefaultBuyPrice} from '../../Redux/Action/DealCenterAction'
...
export default class TradingItem extends React.Component {
    ...
    sliderChange = (num) => {
        ...
        number.value = initNumber(num * rmbBalance / amount / 100, pointNum)
        this.setState({
            sliderValue: num,
            totalPrice: initNumber(amount * number.value, pointPrice),
            number: initNumber(num * rmbBalance / amount / 100, pointNum)
        })
        // } else {// 市价
    ...
    //    const { rmbBalance } = this.props.personalAccount
    //    const { amountHighLimit, amountLowLimit } = this.props.cates.current
    //    if(parseInt(number) < amountLowLimit || parseInt(number) > amountHighLimit || parseInt(number) > initNumber(rmbBalance / currentAmount, 4)) {// 大于最大，小于最小，大于自己最大可买
    ...
    //            fee: initNumber(currentAmount * notLimit * 0.002, 4)
    //        })
    //    }
    //
    // }
    ...
    componentDidMount() {
        ...
        this.setState({
            priceValue: initNumber(buyPrice, 4)
        })
    }
        ...
        return (
            <div className="trading-item fl">
                ...
                <div className="clearfix">
                    <p>最佳买价：<span
                        className="green">{buyPrice ? formatNumber(buyPrice, pointPrice) : formatNumber(currentAmount, pointPrice)}</span>（CNY）
                    </p>
                    <p>可用：<span>{formatNumber(rmbBalance, pointPrice)}（CNY）</span><span className="approximate">≈</span><span>{!(rmbBalance / currentAmount) ? formatNumber(0, pointNum) : formatNumber(rmbBalance / currentAmount, pointNum)}（{currencyNameEn}）</span>
                    </p>
                ...
                {limit ? <div className="input-item-box clearfix ofh">
