...
import {checkWeiChatPay} from '../../Redux/Action/PayAction'
import WeChatPaySuccess from './WeChatPaySuccess'
...
export default class WeChatPayForCode extends React.Component {
    //准备在这里请求一下后台接口，5秒一次，在这里会得到充值失败还有成功的返回值，
    // 不管成功还是失败就让他的子集消失
    componentDidMount() {
        const {dispatch} = this.props
        const {orderNumber} = this.props.payWeiChat

        timer = setInterval(function () {
            dispatch(checkWeiChatPay(dispatch, orderNumber))
        }, 5000)

    }

    componentWillReceiveProps(nextProps) {
        if (!nextProps.weiChatInfo.closeBox) {
            clearInterval(timer)
        }
    }
    ...
    cancelFunc() {
        //给父级传一个参数，重新放回登录页面
        const {dispatch} = this.props
        dispatch({type: 'CANCEL_PAY_WEICHAT_STATUS'})
        //点击取消订单的时候重新调取列表
        let params = {
            status: 0,
            start: 1,
    ...
    // weChatPayForCode
    render() {
        const {show, img, payMoney, orderNumber} = this.props.payWeiChat
        const {payStatus, closeBox, actual_amount} = this.props.weiChatInfo
        return (
            <div className={show ? "hide weChatPayFor" : "show weChatPayFor"}>
                ...
                {closeBox ? "" : <WeChatPaySuccess dispatch={this.props.dispatch}
